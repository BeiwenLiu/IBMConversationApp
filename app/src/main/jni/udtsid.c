...
jlong Java_com_sensory_TrulyHandsfreeSDK_Console_udtsid(JNIEnv* inst,
...
#endif
{
  ...
  /* Create SDK session */
    ...
    for (uIdx = 0; uIdx < numUsers; uIdx++) {
      if ((!UDT_CACHE) || (!existingCache)){
		    ...
		    for (eIdx = 0; eIdx < numEnroll; eIdx++) {
				...
				/* Pipelined recognition */START_REC();
		...
		/* EPQ is recommended with UDT/UDP */
		if (!thfRecogConfigSet(ses,UDTrecog,REC_EPQ_ENABLE,1))
	...
	if (!thfRecogConfigSet(ses, recogTrigger, REC_EPQ_ENABLE, USE_EPQ))
	  THROW("thfRecogConfigSet: epq");
