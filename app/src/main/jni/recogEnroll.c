...
void printEnrollFeedback(void *cons, void *inst, void *job, thf_t *ses, thfEnrollmentCheck_t *enrollmentCheckResult) {
  int  eIdx;
  int  rIdx;
  int  width[] = {9, 9, 11, 11, 9, 9};  /* output column widths */
  char str[MAXSTR];
  char *nomark="  ";
  char *mark="**";
  sprintf(str, "\n              %-11s%-11s%-13s%-13s%-11s%-11s", 
          "ENERGY_MIN", "ENERGY_VAR", "SIL_BEG(ms)", "SIL_END(ms)", 
          "SNR", "REC_VAR");
  DISP(cons, str);
  dispn(cons,"Check Thresh  ");
  for (rIdx  = CHECK_TRIGGER_ENERGY_MIN; rIdx <= CHECK_TRIGGER_RECORDING_VARIANCE; rIdx++) {
    sprintf(str, "%s%-*.2f", nomark, width[rIdx],enrollmentCheckResult->enroll[0].thresh[rIdx]+EPSILON);
    dispn(cons, str);
  }
  DISP(cons, "");
  for (eIdx = 0; eIdx < enrollmentCheckResult->numEnroll; eIdx++) {
    ...
    dispn(cons, str);
    for (rIdx  = CHECK_TRIGGER_ENERGY_MIN; rIdx<=CHECK_TRIGGER_RECORDING_VARIANCE; rIdx++) {
      sprintf(str, "%s%-*.2f", enrollmentCheckResult->enroll[eIdx].pass[rIdx]?nomark:mark, 
	      width[rIdx], enrollmentCheckResult->enroll[eIdx].score[rIdx]+EPSILON);
      dispn(cons, str);
    }
    DISP(cons, "");
  }
  sprintf(str, "\n              %-11s%-11s%-13s%-13s%-11s%-11s", 
          "POOR_REC.", "CLIP(fr)", "VOWEL_DUR", "REPETITION", 
          "SILENCE",   "SPOT");
  DISP(cons, str);
  dispn(cons, "Check Thresh  ");
  for (rIdx  = CHECK_TRIGGER_POOR_RECORDINGS_LIMIT; rIdx <= CHECK_TRIGGER_SPOT; rIdx++) {
    sprintf(str, "%s%-*.2f", nomark, width[rIdx-CHECK_TRIGGER_POOR_RECORDINGS_LIMIT],
	    enrollmentCheckResult->enroll[0].thresh[rIdx]+EPSILON);
    dispn(cons, str);
  }
  DISP(cons, "");
  for (eIdx = 0; eIdx < enrollmentCheckResult->numEnroll; eIdx++) {
    ...
    for (rIdx=CHECK_TRIGGER_POOR_RECORDINGS_LIMIT; rIdx <= CHECK_TRIGGER_SPOT; rIdx++) {
      sprintf(str, "%s%-*.2f", enrollmentCheckResult->enroll[eIdx].pass[rIdx]?nomark:mark, 
	      width[rIdx-CHECK_TRIGGER_POOR_RECORDINGS_LIMIT], 
	      enrollmentCheckResult->enroll[eIdx].score[rIdx]+EPSILON);
      dispn(cons, str);
    }
    DISP(cons, "");
  }
...
#endif
{
  ...
  for (; uIdx < numUsers; uIdx++) {
    ...
    printEnrollFeedback(cons, inst, job, ses, enrollmentCheckResult);
