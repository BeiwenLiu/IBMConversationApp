...
# define GET_NEXT_BUFFER() audioGetNextBuffer()
...
jlong Java_com_sensory_TrulyHandsfreeSDK_Console_SpeakerVerification
...
#endif
{
  ...
  if(!(ses=thfSessionCreate())) {
    panic(cons,"thfSessionCreate","could not find dll or out of memory",0);
    return 1;
  }  
  ...
  if (ALWAYS_ENROLL || recogTest == NULL || searchTest == NULL) {
    ...
    if(!thfRecogInit(ses, recogEnroll, searchEnroll, RECOG_KEEP_WAVE_WORD_PHONEME))
          THROW("thfRecogInit");
	...
	/* EPQ is recommended with fixed-phrase speaker verification */
    if (!thfRecogConfigSet(ses,recogEnroll,REC_EPQ_ENABLE,1))
  ...
  if(!thfRecogInit(ses,recogTest,searchTest,RECOG_KEEP_NONE)) 
    THROW("thfRecogInit");
